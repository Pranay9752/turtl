...
const FollowUsers = () => {
  ...
  const removeRequest = async (index, address) => {
    ...
    await userInter.methods.follow(address).send({ from: window.ethereum.selectedAddress });
  };

  useEffect(() => {
    function removeElementsFromArray(a, b, c) {
      const filteredArray = a.filter(item => !b.includes(item));
      const removedC = filteredArray.filter(item => item.toLowerCase() !== c.toLowerCase());
      return removedC
    }
    ...
    const fetchFollower = async () => {
      try {
        ...
        const allUser = await userRegistry.methods.getAllusers().call().then(
          (users) => {
        ...
        const usersArray = removeElementsFromArray(allUser, followers_add, userAdd)
    ...
    <div >
      ...
      <ul className="max-w-md divide-y m-4 divide-gray-200 dark:divide-gray-700">
        {requests.map((item, index) => (
          ...
          <FollowUser key={index} address={item} index={index} onRemoveRequest={removeRequest} />
        ))}
      </ul>
    </div>
  );
