...
function Exp3() {
    ...
    function generateSubtree(leftKey, rightKey, followerPublicKeys, encryptedContentKey) {
      ...
      const leftSubtree = generateSubtree(...splitKey(leftKey), filteredPublicKeys.slice(0, filteredPublicKeys.length / 2), encryptedContentKey);
      const rightSubtree = generateSubtree(...splitKey(rightKey), filteredPublicKeys.slice(filteredPublicKeys.length / 2), encryptedContentKey);
  
      return {
        left: leftSubtree,
        right: rightSubtree,
        followers: filteredPublicKeys.map(publicKey => {
    ...
    if (user && contentKey) {
      ...
      const tree = generateSubtree(leftRootKey, rightRootKey, followerPublicKeys.map(follower => follower.publicKey), encryptedContentKey);
    }
